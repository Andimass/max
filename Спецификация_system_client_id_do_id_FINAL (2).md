...
В параметр `system_client_id` подставляется `do_id`. Значение берётся из таблицы `employee_py_nob` (поле `do_id`), оно присутствует в строке и не требует дополнительного JOIN — шаг JOIN с `epk_person_relation.snils` для получения СНИЛС удаляется.

Механика хранения и отправки: в таблицу `employee_py_nob` данные сохраняются по разным `do_id` (отдельная строка на связку do_id + ms_base_type + tax_rate + datefor). При формировании сообщения отправки операции складываются (группируются) по `epk_id`, поэтому в один элемент `epk_operations` попадает одно значение `system_client_id`. Если под одним `epk_id` несколько `do_id`, в поле подставляется `do_id` одной из сложенных строк; правило выбора конкретного do_id непринципиально, так как `system_client_id` — справочный идентификатор для сверок на стороне клиента и Мастер Системой для расчёта не используется.

Остальные шаги алгоритма формирования сообщения (фильтрация по статусу, группировка по `epk_id`, группировка по `operation_id`, раскладка по `rate / base / tax`) остаются без изменений.

### 3.2. Группировка

Группировка операций остаётся по `epk_id` — структура сообщения сохраняется, `epk_operations` по `do_id` принудительно не разворачивается. Мастер Система рассчитывает НОБ в разрезе `epk_id` и `system_client_id` для расчёта не использует. Изменяется только содержимое поля `system_client_id` (СНИЛС → do_id).
...
В схеме ответа поле `system_client_id` уже имеет тип string (maxLength 100), доработка не требуется. do_id (36 символов) помещается в maxLength 100 с запасом. Обработка ответа на это поле не завязана: Мастер Система рассчитывает НОБ по epk_id и не использует `system_client_id`, сверка ответа на стороне ReportsTax (раздел 2.2.2) также выполняется по epk_id. Поэтому смена значения поля на обработку ответа не влияет.

### 3.4. Маппинг поля

| Тэг | Схема | Тип AS-IS | Тип TO-BE | Обязательность | Источник TO-BE | Комментарий |
|---|---|---|---|---|---|---|
| system_client_id | Запрос (2.2.1) | Число (number, max 19 девяток) | Строка (string, maxLength 255) | 1 | employee_py_nob.do_id | Справочный ИД клиента для сверок (МС не использует для расчёта). Было — СНИЛС из epk_person_relation.snils, стало — do_id (UUID). При складывании по epk_id берётся do_id одной из строк, правило выбора непринципиально |
| system_client_id | Ответ (2.2.2) | Строка (string, maxLength 100) | Строка (string, maxLength 100) | 1 | — | Уже string, изменения не требуются. МС поле не использует (считает по epk_id), do_id (36 симв.) влезает в 100 |
...
3. **Схема ответа — доработка НЕ требуется.**
   В схеме ответного сообщения (топик `DST.MSNDFL.OPERATIONS_TO_PULS.V1`) поле `system_client_id` уже имеет тип string (maxLength 100). Менять не нужно. Обработка ответа на это поле не завязана — сверка идёт по epk_id (раздел 2.2.2), МС поле не использует. Доработок по ответу не требуется.
